...
function gqCensus(): array
{
    return [
        ...
        'orders.csv' => ['entity' => 'orders', 'columns' => [
            ...
            /*
             * EXPORTED AND NOT LANDED, ON PURPOSE, AND THAT IS THE IMPROVEMENT.
             *
             * `_order_key` is the token in every "order received" and "view
             * order" link WooCommerce ever emailed. Before this lane it was in
             * neither the export nor any report channel: KBB_Export_Orders_Source
             * read it under both storages and the orders STAGE did not list the
             * column, so the value was fetched and dropped on the floor with
             * nothing anywhere able to say so -- the exact silent shape this
             * census exists to end.
             *
             * The stage lists it now, which costs one word and moves it from
             * invisible to named: `orders` has no column for it, so the runner's
             * ignored-column line reports it. Whether it should LAND is the
             * owner's decision, because it only buys serving five-year-old
             * order-email links after the cutover, and his own account area does
             * not need it -- a signed-in customer is unaffected either way.
             */
            'order_key' => GQ_CONSOLIDATED,
