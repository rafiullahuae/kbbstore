...
it('gives the integrator four anchors that each occur exactly once', function () {
    /*
    ...
    foreach ($sections as $i => $section) {
        ...
        expect(substr_count($app, $anchor))->toBe(
            1,
            'block '.($i + 1).' anchor occurs '.substr_count($app, $anchor).' times in app.blade.php, not once'
        );

        expect(trim($replacement))->not->toBe('', 'block '.($i + 1).' has an empty replacement');

        $app = str_replace($anchor, $replacement, $app);
    }
...
it('leaves app.blade.php untouched in this branch', function () {
    // The blocks are a document, not an edit. A lane that edited that file
    // would collide with whichever other lane owns it this cycle.
    $app = (string) file_get_contents(resource_path('views/admin/app.blade.php'));

    expect($app)->not->toContain('translation-screens')
        ->and($app)->not->toContain("sec:'Translation'");
});
