...
/**
 * Mounts routes/manual-orders-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the manual-order
 * routes ship in their own file with the require line in its header. Nothing
 * dispatches to them until someone wires that up, which leaves the gap every
 * lane before this one hit: a mistake in the guard would not surface until a
 * package shipped an endpoint that hands every customer's name, email, phone
 * and address to anyone who asks — and, here, lets them create orders.
 *
 * Deliberately a sibling of Tests\Support\OrdersAdminRoutes rather than a
 * shared base class, for the reason that file gives: it belongs to another lane
 * and is not this one's to change.
 */
final class ManualOrdersAdminRoutes
{
    /**
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/manual-orders-admin.php'));
    ...
    /**
     * Every route this lane's file registers, so a test asserts over all of
     * them rather than over a list it has to remember to update.
     *
    ...
    public static function paths(): array
    {
        return collect(self::registered())
            ->map(fn ($r) => [collect($r->methods())->first(fn ($m) => $m !== 'HEAD'), '/' . $r->uri()])
            ->values()
            ->all();
    }
}
