...
/**
 * Mounts routes/invoices-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the invoice
 * routes ship in their own file with the require line in the header. Nothing
 * dispatches to InvoiceController until someone else wires it up, which leaves
 * the same gap Lane D hit with payments, Lane N with the catalogue, Lane T with
 * customers and Lane V with orders: a mistake in the guard would not surface
 * until a package shipped an endpoint that prints a buyer's name, street
 * address, phone number and order value on a page built for a browser to show.
 *
 ...
 * goes in one array below, and InvoiceDocumentTest reads the middleware back
 * off the REGISTERED routes rather than trusting this file's intent.
 *
 * Deliberately a sibling of Tests\Support\OrdersAdminRoutes rather than a
 * shared base class: that file belongs to another lane and is not this one's
 * to change.
 */
final class InvoiceAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/invoices-admin.php'));
