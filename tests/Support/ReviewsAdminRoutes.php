...
/**
 * Mounts routes/reviews-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the review
 * moderation routes ship in their own file with the require line in the header.
 * Nothing dispatches to the controller until someone else wires it up, which
 * leaves the gap Lane D hit with payments and Lane T with customers: a mistake
 * in the guard would not surface until a package shipped an endpoint handing
 * every reviewer's email address, and the reviewer's IP, to anyone who asks.
 *
 ...
 * change.
 */
final class ReviewsAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/reviews-admin.php'));
     ...
     *
     * Filtered by CONTROLLER, not by URI alone: routes/web.php already has
     * three /admin-api/reviews routes pointing at AdminController, and those
     * are another lane's to guard.
     *
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'ReviewsApiController'))
            ->values()
            ->all();
    }
}
