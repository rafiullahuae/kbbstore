...
/**
 * Mounts routes/html-blocks-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the block routes
 * ship in their own file with the require line in its header. Nothing
 ...
 * package shipped a write endpoint whose payload is HTML rendered unescaped
 * into storefront pages.
 *
 ...
 */
final class HtmlBlocksAdminRoutes
{
    /**
     ...
     * appending to it, so a chained form registers only the last one -- a
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/html-blocks-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/blocks'))
            ->values()
            ->all();
    }
