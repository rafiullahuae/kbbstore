...
/**
 * Mounts routes/customers-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the customer
 ...
 * same gap Lane D hit with payments and Lane N with the catalogue: a mistake in
 * the guard would not surface until a package shipped an endpoint that hands
 * every shopper's email, phone and home address to anyone who asks.
 *
 ...
 * reversed, no guard at all while still looking guarded in the source. A guard
 * test written against that harness passes against nothing. The whole stack
 * goes in one array below, and assertGuarded() reads the middleware back off
 * the registered routes so the harness itself is checked rather than trusted.
 *
 * Deliberately a sibling of Tests\Support\CatalogAdminRoutes rather than a
 * shared base class: that file belongs to another lane and is not this one's to
 * change.
 */
final class CustomersAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/customers-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/customers/')
                || $r->uri() === 'admin-api/customers')
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'CustomersApiController'))
            ->values()
            ->all();
    }
}
