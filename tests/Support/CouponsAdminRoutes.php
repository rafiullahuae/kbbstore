...
/**
 * Mounts routes/coupons-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the coupon
 ...
 * package shipped an endpoint that hands out the email address of every
 * shopper who redeemed a given code.
 *
 * A sibling of Tests\Support\ManualOrdersAdminRoutes rather than a shared base
 * class, for the reason that file gives about OrdersAdminRoutes: it belongs to
 * another lane and is not this one's to change.
 */
final class CouponsAdminRoutes
{
    /**
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/coupons-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/coupons'))
            ->values()
            ->all();
    }
