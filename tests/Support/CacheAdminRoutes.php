...
/**
 * Mounts routes/cache-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the four routes
 ...
 * would otherwise leave the capability guard on a button that clears a live
 * shop's compiled routes untested until a package shipped it.
 *
...
final class CacheAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/cache-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => $r->uri() === 'admin-api/cache' || str_starts_with($r->uri(), 'admin-api/cache/'))
            ->values()
            ->all();
    }
}
