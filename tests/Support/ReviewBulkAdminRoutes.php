...
/**
 * Mounts routes/review-bulk-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the bulk-review
 ...
 * guard would not surface until a package shipped an endpoint that lets anyone
 * write review text onto any product page on the storefront — and, through
 * Store\ProductController::reviewSummary() and App\Support\Seo, into the
 * schema.org aggregateRating this site publishes to Google.
 *
 ...
 * change.
 */
final class ReviewBulkAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/review-bulk-admin.php'));
     ...
     *
     * Filtered by CONTROLLER, not by URI: routes/web.php already carries other
     * /admin-api/review* routes belonging to other lanes, and those are theirs
     * to guard.
     *
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'ReviewBulkApiController'))
            ->values()
            ->all();
    }
}
