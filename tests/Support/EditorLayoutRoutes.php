...
/**
 * Mounts routes/editor-layout-admin.php the way the integrator is told to.
 * (Lane AS)
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the routes ship
 * in their own file with the require line in its header. Nothing dispatches to
 * the controller until someone wires it up, which leaves the gap every other
 * lane here has hit in turn: a mistake in the guard would not surface until a
 * package shipped an endpoint that any anonymous caller could write rows
 * through, one per admin account.
 *
 ...
 * goes in one array below, and ProductEditorLayoutTest reads the middleware
 * back off the REGISTERED routes rather than trusting this file's intent.
 *
 * A deliberate sibling of Tests\Support\ProductEditorRoutes rather than a
 * shared base class or an edit to it: that file belongs to Lane AO and is not
 * this one's to change.
 */
final class EditorLayoutRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        // Idempotent: one test may wire this lane's file alongside another's.
        foreach ($router->getRoutes()->getRoutes() as $route) {
            if (str_starts_with($route->uri(), 'admin-api/editor-layout')) {
                return;
            }
        }
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/editor-layout-admin.php'));
     ...
     *
     * Filtered by path prefix AND by controller, for the reason Lane AK's
     * equivalent spells out: a prefix filter alone can sweep in a sibling
     * lane's routes and count them as this lane's work, and the route-count
     * assertion would then pass while this file registered nothing at all.
     *
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/editor-layout'))
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'AdminScreenLayoutApiController'))
            ->values()
            ->all();
    }
}
