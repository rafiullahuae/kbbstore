...
/**
 * Mounts routes/product-editor-admin.php the way the integrator is told to.
 *
 ...
 * every other lane here has hit in turn: a mistake in the guard would not
 * surface until a package shipped an endpoint letting anyone who asks rewrite
 * the shop's catalogue — and two of these routes can put a product on the
 * storefront in one request.
 *
...
final class ProductEditorRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        foreach ($router->getRoutes()->getRoutes() as $route) {
            if (str_starts_with($route->uri(), 'admin-api/product-editor-')) {
                return;
            }
        }
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/product-editor-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/product-editor-'))
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'ProductEditorApiController'))
            ->values()
            ->all();
    }
}
