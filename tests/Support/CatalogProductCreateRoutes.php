...
/**
 * Mounts routes/catalog-product-create-admin.php the way the integrator is told
 * to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the Add-product
 ...
 * same gap Lane D hit with payments, Lane N with the catalogue, Lane T with
 * customers, Lane V with orders and Lane AF with this very screen: a mistake in
 * the guard would not surface until a package shipped an endpoint that lets
 * anyone who asks add a product to the shop, or replace a bestseller's
 * photograph with anything they like.
 *
 ...
 * goes in one array below, and AdminCatalogProductCreateTest reads the
 * middleware back off the REGISTERED routes rather than trusting this file's
 * intent.
 *
 * A deliberate sibling of Tests\Support\CatalogProductsAdminRoutes rather than
 * a shared base class or an edit to it: that file belongs to Lane AF and is not
 * this one's to change. The duplication is four lines and the alternative is
 * two lanes editing one harness.
 */
final class CatalogProductCreateRoutes
{
    ...
    public static function wire(Application $app): void
    {
         ...
         * still a refusal, so a guard test would pass for the wrong reason.
         */
        $app->make(\Illuminate\Contracts\Http\Kernel::class);

        $router = RouteFacade::getFacadeRoot();

        // Idempotent: a single test may wire both this lane's file and Lane
        // AF's, and registering the same path twice would make the route count
        // assertions meaningless.
        foreach ($router->getRoutes()->getRoutes() as $route) {
            if (str_starts_with($route->uri(), 'admin-api/catalog-product-')) {
                return;
            }
        }
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/catalog-product-create-admin.php'));
     ...
     *
     * Filtered by path prefix AND by controller. The prefix alone is not
     * enough: `admin-api/catalog-product-` is a string prefix of Lane AF's
     * `admin-api/catalog-products-` too, so without the controller filter every
     * one of that lane's eight routes would be swept in here and silently
     * counted as this lane's work — and the route-count assertion would pass
     * while this file registered nothing at all.
     *
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/catalog-product-'))
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'CatalogProductCreateApiController'))
            ->values()
            ->all();
    }
}
