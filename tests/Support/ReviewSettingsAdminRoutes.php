...
/**
 * Mounts routes/review-settings-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the Review
 * Settings routes ship in their own file with the require line in the header.
 * Nothing dispatches to the controller until someone else wires it up, which
 * leaves the gap Lane AM hit with the moderation screen and Lane T with
 * customers: a mistake in the guard would not surface until a package shipped
 * an anonymous switch for a live store's review section and its anti-spam
 * ceiling.
 *
 ...
 * change.
 */
final class ReviewSettingsAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/review-settings-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'ReviewSettingsApiController'))
            ->values()
            ->all();
    }
}
