...
/**
 * Mounts routes/reviews-screens-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the ten routes
 * for the four Reviews screens ship in their own file with the require line in
 * its header. Nothing dispatches to those controllers until someone else wires
 * it up, which leaves the gap Lane AM hit with the moderation screen and Lane T
 * with customers: a mistake in the guard would not surface until a package
 * shipped an unauthenticated CSV of every reviewer's email address and an
 * unauthenticated form for writing five-star reviews into a live store.
 *
 ...
 *
 * Deliberately a sibling of Tests\Support\ReviewsAdminRoutes and
 * Tests\Support\ReviewSettingsAdminRoutes rather than a shared base class:
 * those files belong to other lanes and are not this one's to change.
 */
final class ReviewsScreensAdminRoutes
{
    /** The exact stack routes/web.php's admin-api group applies. */
    public const STACK = ['web', 'auth:admin', NoStoreAdminApi::class];

    /** The controllers this lane's route file registers. */
    public const CONTROLLERS = [
        'ReviewsIoApiController',
        'ReviewBadgeApiController',
        'ReviewAssignApiController',
    ];
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/reviews-screens-admin.php'));
     ...
     *
     * Filtered by CONTROLLER, not by URI: /admin-api/reviews already carries
     * three AdminController routes and five ReviewsApiController ones, and
     * those are other lanes' to guard.
     *
