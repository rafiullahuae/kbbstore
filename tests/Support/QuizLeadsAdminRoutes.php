...
/**
 * Mounts routes/quiz-leads-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the one route it
 * adds ships in its own file with the require line in that file's header.
 * Nothing dispatches to the controller until someone else wires it up, which
 * would otherwise leave the guard untested until a package shipped it.
 *
 ...
 * to change.
 */
final class QuizLeadsAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/quiz-leads-admin.php'));
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ->filter(fn ($r) => str_starts_with($r->uri(), 'admin-api/quiz-leads/'))
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'updateQuizLead'))
            ->values()
            ->all();
    }
}
