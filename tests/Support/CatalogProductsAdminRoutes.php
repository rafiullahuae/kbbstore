...
/**
 * Mounts routes/catalog-products-admin.php the way the integrator is told to.
 *
 * CLAUDE.md forbids this lane from editing routes/web.php, so the Catalog →
 * Products routes ship in their own file with the require line in the header.
 * Nothing dispatches to the controller until someone else wires it up, which
 * leaves the same gap Lane D hit with payments, Lane N with the catalogue, Lane
 * T with customers and Lane V with orders: a mistake in the guard would not
 * surface until a package shipped an endpoint that lets anyone who asks rewrite
 * the shop's own prices, unpublish its catalogue, or download every SKU.
 *
 ...
 * in one array below, and AdminCatalogProductsTest reads the middleware back off
 * the REGISTERED routes rather than trusting this file's intent.
 *
 * Deliberately a sibling of Tests\Support\OrdersAdminRoutes rather than a shared
 * base class: that file belongs to another lane and is not this one's to change.
 */
final class CatalogProductsAdminRoutes
{
    ...
    public static function wire(Application $app): void
    {
        ...
        // Idempotent: the dialect guard wires several lanes' route files in one
        // test, and registering the same path twice would make the route count
        // assertions meaningless.
        foreach ($router->getRoutes()->getRoutes() as $route) {
            if (str_starts_with($route->uri(), 'admin-api/catalog-products-')) {
                return;
            }
        }
        ...
        RouteFacade::middleware(self::STACK)
            ->prefix('admin-api')
            ->group(base_path('routes/catalog-products-admin.php'));
     ...
     *
     * Filtered by path prefix AND by controller, so the catalogue routes
     * routes/web.php already registers against this same controller
     * (/admin-api/catalog/products and its toggle-featured) are not swept in and
     * silently counted as this lane's work.
     *
    ...
    public static function registered(): array
    {
        return collect(RouteFacade::getRoutes()->getRoutes())
            ...
            ->filter(fn ($r) => str_contains((string) $r->getAction('controller'), 'CatalogProductsApiController'))
            ->values()
            ->all();
    }
}
