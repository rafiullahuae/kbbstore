...
|------------------------------------------------------------------------------
|
| LIVE. routes/web.php requires this file; the header below was struck in
| the same commit that mounted it, because a route file still calling
| itself unmounted is a lie the next lane reads and believes, and
| RouteFileHeadersTest fails by name if it comes back.
|
| Mounted at top level in routes/web.php, beside the other top-level requires
...
|
| ▲ `import-chain` is in PageController::RESERVED_SLUGS. It has to be: this
| endpoint carries no `auth:admin`, and articles live at the SITE ROOT, so an
| article published at that slug would shadow it. RootSlugCollisionTest caught
| that within a minute of the route being mounted.
|
|==============================================================================
