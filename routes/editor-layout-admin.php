...
|------------------------------------------------------------------------------
|
| WIRED. Mounted from routes/web.php inside the existing admin-api group — the
| one already carrying `auth:admin` and NoStoreAdminApi — beside the product
| editor's own require, because this is that screen's preference store.
|
| The header carried an unmounted claim while the lane built it, with a note
| asking the integrator to correct it on mounting. Done, and worth recording
| why the note was there: tests/Feature/RouteFileHeadersTest.php fails any
| route file required from web.php or api.php that still claims to be
| unmounted. That test exists because fourteen files drifted into exactly that
| state and a reader concluded a live endpoint was dead code.
|
| The line that mounts it:
|
