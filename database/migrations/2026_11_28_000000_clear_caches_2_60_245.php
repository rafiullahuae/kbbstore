...
/**
 * Clear the compiled views for 2.60.245.
 *
 * WHAT IS STALE IF THIS DOES NOT RUN: the checkout, most of it.
 *
 *   - store/checkout.blade.php moves Full name into Contact, marks phone
 *     required, and replaces the five Shipping address fields with an include.
 *   - partials/checkout-address.blade.php IS that section, and it renders the
 *     four hidden inputs the order is posted with. A stale compiled checkout
 *     that still draws the typed fields is harmless; one that has the include
 *     compiled against a missing partial is not.
 *   - partials/address-sheet.blade.php is new: the delivery-address sheet,
 *     shared by the cart page and the checkout.
 *   - store/cart.blade.php and cart-squeeze.blade.php stop carrying the sheet
 *     themselves and include it instead. A stale pair of those is a cart page
 *     with two copies of the sheet's stylesheet, or none.
 *
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (app()->runningInConsole()) {
            echo "Cleared {$cleared} compiled files; the checkout takes its delivery address from\n";
            echo "the same list the cart page keeps, and the name sits with the email and phone.\n";
        }
    }
