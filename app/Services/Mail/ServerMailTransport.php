...
class ServerMailTransport extends AbstractTransport
{
    ...
    {
        /*
         * Never hand a message to a real mail program from a test run.
         *
         * This is not belt-and-braces. phpunit.xml sets MAIL_MAILER=array, but
         * that guards the DEFAULT mailer only, and nothing here sends through
         * the default mailer: MailConfigurator writes its own `kbb` mailer into
         * the config at runtime and OrderMailer sends through that by name. So
         * the array transport is never reached, and eighteen checkout tests
         * were reaching this line and calling mail() for real. On this
         * container and on CI that is merely noisy — there is no MTA, mail()
         * returns false, OrderMailer swallows it. On a developer machine that
         * HAS one, those are real messages to whatever addresses the fixtures
         * happen to carry, sent by `vendor/bin/pest`.
         *
         * The guard sits here rather than in the test bootstrap because
         * MailConfigurator::refresh() re-registers this transport with
         * Mail::extend() on every refresh, which would overwrite anything a
         * TestCase had registered. It costs production nothing: outside a test
         * run the condition is false on the first call and never consulted
         * again for that message.
         *
         * It does not make anything vacuous. Every test that asserts on what
         * reaches mail() — the argument spies in ServerMailDefaultTest — does
         * it by subclassing and OVERRIDING this method, so it never runs this
         * body at all. All the header surgery above still executes for real.
         */
        if (app()->runningUnitTests()) {
            self::$lastTestDelivery = compact('to', 'subject', 'headers', 'params')
    ...
    /**
     * What the last test-run delivery would have handed to mail().
     *
     * Recorded rather than discarded so the suppression above is observable:
     * a test can assert the guard fired, which is how we know a green suite is
     * green because nothing was sent and not because nothing was rendered.
     *
     * @var array<string, mixed>|null
     */
    public static ?array $lastTestDelivery = null;
