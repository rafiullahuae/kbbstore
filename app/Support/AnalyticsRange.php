...
final class AnalyticsRange
{
     ...
     *
     * Delegated to App\Support\StoreTime, which is the shop's clock for the
     * whole panel: the dashboard's "today", the date on an invoice, the date on
     * a customer's confirmation email, and now these bars. The owner sets it
     * once in Store -> Business Details (`store_timezone`, default Asia/Dubai),
     * and this screen cannot disagree with the rest of the admin about what day
     * it is — which was the entire failure this file and StoreTime were each
     * written to fix, independently, at the same time.
     *
     * It is deliberately NOT `config('app.timezone')`. Moving that would not
     * convert stored timestamps, it would REINTERPRET them: a row written as
     * 21:30 UTC starts being read as 21:30 Dubai, four hours later than the
     * instant that was recorded, so every historical order silently shifts and
     * a re-save then writes Dubai wall-clock into a UTC column for real. See
     * StoreTime's header; a test pins the distinction.
     *
     * Falls back to the storage clock, so a shop with no setting and no
     * StoreTime available bucket-groups exactly as it did before either
     * existed.
     */
    public static function timezone(): string
    {
        return self::known(StoreTime::zone() ?: self::storageTimezone());
    }
