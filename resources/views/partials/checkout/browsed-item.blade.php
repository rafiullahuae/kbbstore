...
@endphp
<div class="bitem"><a class="bth" href="{{ $bp->url() }}" style="{{ $thumb }}"></a><div class="binfo"><a class="bn" href="{{ $bp->url() }}">{{ $bp->name }}</a><div class="bp">@if ($bp->isOnSale())<del aria-hidden="true">{!! Money::format((int) $bp->price) !!}</del><ins aria-hidden="true">{!! Money::format($bp->effectivePrice()) !!}</ins>@else{!! Money::format($bp->effectivePrice()) !!}@endif</div></div><button type="button" class="baddbtn" data-kbb-checkout-add="{{ $bp->id }}" data-kbb-add="{{ $bp->id }}" data-price="{{ number_format($bp->effectivePrice() / 100, 2, '.', '') }}" data-name="{{ $bp->name }}" aria-label="Add {{ $bp->name }} to cart">Add</button></div>