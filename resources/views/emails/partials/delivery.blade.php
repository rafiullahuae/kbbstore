...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%;border-collapse:collapse;margin:22px 0 0;">
    <tr>
        <td style="vertical-align:top;padding:0 12px 0 0;width:50%;font-size:14px;line-height:1.5;">
            <div style="font-size:10.5px;letter-spacing:.09em;text-transform:uppercase;color:{{ $c['muted'] }};font-weight:700;margin-bottom:6px;">{{ __('email.delivery.address_heading') }}</div>
            @forelse ($order['address'] as $line)
                <div style="color:{{ $c['ink'] }};">{{ $line }}</div>
            @empty
                <div style="color:{{ $c['ink2'] }};">{{ __('email.delivery.not_recorded') }}</div>
            @endforelse
        </td>
        <td style="vertical-align:top;padding:0 0 0 12px;width:50%;font-size:14px;line-height:1.5;">
            <div style="font-size:10.5px;letter-spacing:.09em;text-transform:uppercase;color:{{ $c['muted'] }};font-weight:700;margin-bottom:6px;">{{ __('email.delivery.method_heading') }}</div>
            <div style="color:{{ $c['ink'] }};">{{ $order['deliveryMethod'] }}</div>

            <div style="font-size:10.5px;letter-spacing:.09em;text-transform:uppercase;color:{{ $c['muted'] }};font-weight:700;margin:14px 0 6px;">{{ __('email.delivery.payment_heading') }}</div>
...
@if ($order['giftNote'] !== '')
    <div style="margin:22px 0 0;padding:13px 15px;background:{{ $c['pinkSoft'] }};border-radius:9px;">
        <div style="font-size:10.5px;letter-spacing:.09em;text-transform:uppercase;color:{{ $c['pinkDeep'] }};font-weight:700;margin-bottom:4px;">{{ __('email.delivery.gift_heading') }}</div>
...
@if ($order['customerNote'] !== '')
    <div style="margin:14px 0 0;padding:13px 15px;background:{{ $c['cream'] }};border-radius:9px;">
        <div style="font-size:10.5px;letter-spacing:.09em;text-transform:uppercase;color:{{ $c['muted'] }};font-weight:700;margin-bottom:4px;">{{ __('email.delivery.note_heading') }}</div>
