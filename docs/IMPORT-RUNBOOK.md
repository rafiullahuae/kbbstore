...
| Files nothing opens | **discarded** | One entry per file in the export folder that no entity opens — `variations.csv`, `tags.csv`, and whatever else the export carries — with its row count. Suppressed under `--only`, where ignoring a file is the point. `coupons.csv`, `reviews.csv`, `refunds.csv` and `order_notes.csv` have each in turn been the worst entry on this list and are now entities; the channel stays for whatever the next export carries. |
